...
abstract class modjvectormapHelper
{
	public static function getjvectormap(&$params)
	{
		...
		foreach ($items as &$item)
		{
			...
			if ($access || in_array($item->access, $authorised))
			{
				...
				$item->linkText = JText::_('MOD_jvectormap_READMORE');
			}
			else {
				$item->link = JRoute::_('index.php?option=com_users&view=login');
				$item->linkText = JText::_('MOD_jvectormap_READMORE_REGISTER');
			}

			$item->introtext = JHtml::_('content.prepare', $item->introtext, '', 'mod_naturemap.content');
			...
			if (!$params->get('image'))
			{
				...
				$item->introtext = preg_replace('/(<img([^>]*)>)/i', '<div class="thumb">$1</div>', $item->introtext);
			}*/
